...
void Tower::Draw() {
  ...
  int draw_y =
      circle->y - (TowerHeight[this->type] - (TowerWidth[this->type] / 2));
  ...
  if (isClicked) {
    al_draw_filled_circle(circle->x, circle->y, circle->r,
                          al_map_rgba(196, 79, 79, 200));
    al_draw_filled_circle(circle->x, circle->y, 2, al_map_rgb(0, 0, 0));
  }
}

void Tower::SelectedTower(int mouse_x, int mouse_y, int type) {
  ...
  al_draw_filled_circle(mouse_x, mouse_y, TowerRadius[type],
                        al_map_rgba(196, 79, 79, 200));
...
bool Tower::DetectAttack(Monster *monster) {
  ...
  if (Circle::isOverlap(this->circle, monster->getCircle())) {
    if (attack_counter == 0) {
      attack = new Attack(this->circle, monster->getCircle(),
                          this->attack_harm_point, this->attack_velocity,
                          this->attack_img);
...
bool Tower::TriggerAttack(Monster *monster) {
